...
def linear_models():
    ...
    # Two dimensional joint equivalent to above
    proc_1 = ts.models.RandomWalk(parameters[-1][0])
    proc_2 = ts.models.RandomWalk(parameters[-1][1])

    joint = ts.joint_process(proc_1=proc_1, proc_2=proc_2)
    joint_ssm = ts.LinearStateSpaceModel(joint, params, torch.Size([2]))
